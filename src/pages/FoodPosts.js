...
const FoodPost = () => {
    ...
    const likePost = async(id) => {
      ...
      if(currentPost){
        const updatedPosts = postList.map((post) => post.id === id ? { ...post, likes: post.likes + 1} : post);
        await updateDoc(postDoc, {likes: currentPost.likes + 1});
        setPostList(posts);
      }
      else {
        console.error(`Post with id ${id} not found in the postList`);
      }
    }
